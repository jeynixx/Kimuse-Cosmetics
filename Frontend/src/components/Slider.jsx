...
export default function Slider({ productData, title, link, listings }) {
    ...
    <section className="mb-5 flex flex-col items-center">
      <div className=" container flex justify-between items-end w-full mb-3 p-0 overflow-visible">
        ...
        <div className=" flex gap-3 items-center ">
          <Link to={link} className=" text-purple-900 hover:text-purple-500">
            <span className="text-purple-900 hover:text-purple-500 text-sm p-1">
              View All
            </span>
          </Link>
      ...
      <div className=" container relative flex flex-col gap-0 p-0">
        ...
        >
          {randomProducts.map((item) => (
            <div
              key={item.id}
              className=" lg:w-1/4 md:w-2/6 w-2/5 mx-1 inline-block overflow-auto ease-in-out duration-300"
            >
              <Crad
                key={item.id}
                id={item.id}
